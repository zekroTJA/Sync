...
import net.schlaubi.spigot.discordcraft.util.EmbedSender;
...
public class MessageListener extends ListenerAdapter{
    ...
    private String getNameFromUUID(String uuid) throws IOException {
        ...
        try (InputStream is = new URL("https://use.gameapis.net/mc/player/profile/" + uuid).openStream()) {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if(event.isFromType(ChannelType.TEXT)){
                ...
                if(args.length > 1){
                    ...
                    if(args[1].equalsIgnoreCase("info")){
                        if(message.getMentionedUsers().size() > 0){
                            if(!MySQL.userExists(message.getMentionedUsers().get(0))){
                            ...
                            if(!MySQL.userExists(event.getAuthor())){
                                EmbedSender.sendEmbed(cfg.getString("Messages.notfound"), event.getTextChannel(), Color.red);
                                return;
                            }
            ...
            } else if(!event.getMessage().getContent().startsWith("+") || cfg.getStringList("Channels.whitelist").contains(event.getChannel().getId())){
                for(Player all : Bukkit.getOnlinePlayers()){
                    all.sendMessage("§9[DISCORD][" + event.getAuthor().getName() + "] > §f " + event.getMessage().getContent());
                }
            }
        }
    }
